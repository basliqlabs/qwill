...
/**
 * The raw SVG string for the link icon.
 */
...
export function rehypeCustomTweaks() {
  return (tree) => {
    visit(tree, { tagName: 'img' }, (node) => {
      const src = node.properties.src
      if (typeof src === 'string' && src.includes('/src')) {
        node.properties.src = src.replace('/src', `${base}/src`)
      }
    })

    visit(tree, { tagName: 'span' }, (node) => {
      const classNames = node.properties.className || []
      if (classNames.includes('icon') && classNames.includes('icon-link')) {
        node.children = [
          {
            type: 'raw',
            value: linkIconSvg
